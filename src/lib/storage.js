// Hantering av profil och säker lagring
export const SafeStore = (() => {
  ...
  function set(v){ if(can){ try{ localStorage.setItem(KEY, JSON.stringify(v)) }catch(e){ mem=v } } else { mem=v } }
...
export function loadProfile(){
  const DEF = {
    ...
    settings:{ perQuiz:10, perExam:20, perQuestionTimerSec:45, examTimerTotalMin:25 },
...
export function saveProfile(p){ SafeStore.set(p) }